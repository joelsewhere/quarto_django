...
{% endif %}'''
    ...
    if 'projects' in file.as_posix():
        ...
        <textarea id="share_link" style="display: none;">{% if not request.user.is_anonymous %}{% call_method request.user 'generate_share_link' request.path %}{% endif %}</textarea>
        """
    
    # final html content
    new_html += str(soup) + """
        {% if 'share' in request.path or "projects" == request.path|split:"/__-2" or request.user.is_anonymous or "/" == request.path %}
    <script>
